<?php
  include_once 'simple_html_dom.php';
    ...
    fetchImages("https://www.att.com/","cnn");
    ...
    function fetchImages($url, $directory) {
      ...
      foreach($html->find('img') as $img) {
        $property = 'data-src';
        preg_match_all($regexImageName, $img->$property, $matches, PREG_SET_ORDER, 0);

        $filename = $dateDirectory."/".$matches[0][0];

        if ( !file_exists($filename) ) {
            //Get the file
            echo('<img src="'.$property.'">');
            $content = file_get_contents($property);
